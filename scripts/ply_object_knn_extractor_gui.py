...
def load_object_as_point_cloud(object_path: Path, num_sample_points: int) -> o3d.geometry.PointCloud:
    ...
    if suffix == ".ply":
        ...
        if len(pcd.points) == 0:
            raise ValueError("The object PLY contains no points.")
    ...
    if suffix == ".obj":
        ...
        if mesh.is_empty():
            raise ValueError("The OBJ file could not be loaded as a mesh.")

        if not mesh.has_triangles():
            raise ValueError("The OBJ file contains no triangles.")
        ...
        if len(pcd.points) == 0:
            raise ValueError("Poisson disk sampling produced no points.")

        return pcd

    raise ValueError("Object must be .obj or .ply.")
...
def extract_nearest_neighbors_from_scan(
    ...
    if len(scan_pcd.points) == 0:
        raise ValueError("The scan PLY contains no points.")
    ...
    if not extracted_indices:
        raise ValueError(
            "No scan points were extracted. "
            "Increase max_distance or verify that object and scan are in the same coordinate system."
        )
    ...
    if not success:
        raise RuntimeError("Failed to write output PLY file.")
...
class ObjectKnnExtractorApp:
    def __init__(self, root):
        ...
        title = tk.Label(
            root,
            text="Extract Object from Scan using KNN",
            font=("Arial", 18, "bold")
        )
        title.pack(pady=15)
        ...
        hint = tk.Label(
            root,
            text="Max distance <= 0 means: no distance filtering",
            fg="gray"
        )
        hint.pack(pady=3)

        drop_text = (
            "Drag & Drop enabled for .ply and .obj files"
            if DND_AVAILABLE
    ...
    def on_drop(self, event):
        ...
        if suffix == ".ply":
            ...
            messagebox.showwarning(
                "Invalid file",
                "Please drop only .ply or .obj files."
            )

    def extract(self):
        try:
            if not self.scan_path.get():
                raise ValueError("Please select a scan PLY file.")

            if not self.object_path.get():
                raise ValueError("Please select an object file (.obj or .ply).")
            ...
            self.status.config(text=f"Done: {output_file}")

            messagebox.showinfo(
                "Success",
                f"Extracted points saved to:\n{output_file}"
            )
