...
def split_ply_by_classes(ply_path: Path, txt_path: Path) -> Path:
    ...
    if "vertex" not in ply:
        raise ValueError("The PLY file contains no 'vertex' element.")
    ...
    if unknown:
        raise ValueError(f"Unknown classes found: {unknown}")
...
class PlySplitterApp:
    def __init__(self, root):
        ...
        drop_text = (
            "Drag & Drop enabled for .ply and .txt"
            if DND_AVAILABLE
    ...
    def on_drop(self, event):
        ...
        if path.suffix.lower() == ".ply":
            ...
            messagebox.showwarning(
                "Invalid File",
                "Please drop only .ply or .txt files."
            )

    def convert(self):
        try:
            if not self.ply_path.get():
                raise ValueError("Please select a PLY file.")
            ...
            self.status.config(text=f"Done. Output: {output_dir}")
            messagebox.showinfo(
                "Success",
