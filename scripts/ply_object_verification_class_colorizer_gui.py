...
CLASS_COLORS_FLOAT = {
    0: (1.0, 0.0, 0.0),  # Red = not verified
    1: (0.0, 1.0, 0.0),  # Green = verified
}
...
def get_scene_id_from_ply(scene_ply: Path) -> str:
    ...
    if not match:
        raise ValueError(
            "The filename must contain scene_001 to scene_007."
        )
...
def get_phase_dir_from_scene_ply(scene_ply: Path) -> Path:
    ...
    if scene_dir.parent.name.lower() != "scans":
        raise ValueError(
            "The PLY file must be located in dataset/phases/phase_XX/scans/scene_XXX."
        )

    phase_dir = scene_dir.parent.parent

    if not phase_dir.name.lower().startswith("phase_"):
        raise ValueError("Could not determine phase_XX from path.")
...
def load_labels(label_file: Path) -> List[Dict]:
    ...
    if not isinstance(records, list):
        raise ValueError("JSON entries must be a list.")
...
def create_sampled_colored_ply_from_obj_components(
    ...
    if not mesh_infos:
        raise ValueError("No OBJ components found for sampling.")
    ...
    if not ok:
        raise RuntimeError(f"Failed to write PLY file:\n{output_file}")
...
def create_colored_merged_obj(
    ...
    if used == 0:
        raise ValueError("No OBJ components could be loaded.")
...
def process_scene_ply(
    ...
    if not labels_by_stem:
        raise ValueError("No components found in label file.")
    ...
    msg = (
        f"Finished for {scene_id}\n\n"
        f"OBJ Output:\n{out_obj}\n"
        f"MTL Output:\n{out_obj.with_suffix('.mtl')}\n\n"
        f"Sampled PLY:\n{out_ply}\n\n"
        f"OBJ components used: {used_obj}\n"
        f"Sampled components: {used_sampled}\n"
        f"Sampling target points: {total_sample_points}\n"
        f"Total labeled components: {len(labels_by_stem)}\n"
    )

    if missing_obj:
        msg += f"\nMissing OBJ files during OBJ merge: {len(missing_obj)}"

    if missing_sampled:
        msg += f"\nMissing OBJ files during sampling: {len(missing_sampled)}"
...
class ObjectVerificationColorizerGui:
    def __init__(self, root):
        ...
        drop_text = (
            "Drag & Drop enabled for .ply"
            if DND_AVAILABLE
            else "Drag & Drop disabled. Install with: pip install tkinterdnd2"
        )
        ...
        self.ply_progress_label = tk.Label(root, text="PLY Progress: 0 / 0")
        ...
        self.obj_progress_label = tk.Label(root, text="OBJ Progress: 0 / 0")
    ...
    def on_drop(self, event):
        ...
        if path.is_file() and path.suffix.lower() == ".ply":
            ...
            messagebox.showwarning(
                "Invalid Drop",
                "Please drop a .ply file.",
            )

    def convert(self):
        try:
            if not self.scene_ply_path.get():
                raise ValueError("Please select a scene PLY.")

            self.ply_progress["value"] = 0
            self.obj_progress["value"] = 0
            self.ply_progress_label.config(text="PLY Progress: 0 / 0")
            self.obj_progress_label.config(text="OBJ Progress: 0 / 0")
    ...
    def update_progress(self, kind, current, total, stem):
        ...
        if kind == "ply":
            ...
            self.ply_progress_label.config(
                text=f"PLY Progress: {current} / {total} ({percent}%)"
            )
            ...
            self.obj_progress_label.config(
                text=f"OBJ Progress: {current} / {total} ({percent}%)"
            )
        elif kind == "sampling":
            self.ply_progress["maximum"] = total
            self.ply_progress["value"] = current
            self.ply_progress_label.config(
                text=f"Sampling Progress: {current} / {total} ({percent}%)"
            )

        self.status.config(text=f"Processing: {stem}")
        self.root.update_idletasks()
